    ...
    if( ( admin_url() == $redirect_to && $rul_allow_post_redirect_override ) || !$rul_allow_post_redirect_override ) {
        ...
        if ( ( user_can( $user, 'wpc_client' ) || user_can( $user, 'wpc_client_staff' ) ) && !user_can( $user, 'manage_network_options' ) )  {
            if ( get_option( 'permalink_structure' ) ) {
                if ( is_multisite() ) {
                    wp_redirect( get_blog_option( get_current_blog_id(), 'home' ) . '/my-hub/' );
                    die();
                } else {
                    wp_redirect( get_option( 'home' ) . '/my-hub/' );
                    die();
                }
            }
        }
...
if (is_admin()) {
    ...
    */
    ...
    function wpc_client_rul_optionsmenu() {
        ...
        <h2>WP-Client Settings</h2>

        <p>From here you can manage a variety of options for the WP-Client plugin.</p>
        <div id="container23">
            <ul class="menu">
                <li id="general"><a href="admin.php?page=wpclients_settings" >General</a></li>
                <li id="clogin"><a href="admin.php?page=custom_login_admin" >Custom Login</a></li>
                <li id="redirects" class="active"><a href="admin.php?page=xyris-login-logout" >Login/Logout Redirects</a></li>
                <li id="skins"><a href="admin.php?page=wpclients_settings&tab=skins" >Skins</a></li>
                <li id="alerts"><a href="admin.php?page=wpclients_settings&tab=alerts" >Login Alerts</a></li>
                <li id="about"><a href="admin.php?page=wpclients_settings&tab=about" >About</a></li>
            </ul>
...
<?php
    ...
    function wpc_client_rul_install() {
        ...
        if( $rul_db_addresses != $wpdb->get_var( 'SHOW TABLES LIKE \'' . $rul_db_addresses . '\'')
            && "{$wpdb->base_prefix}login_redirects" != $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->base_prefix}login_redirects'" ) ) {
